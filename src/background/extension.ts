// import { getDefaultEngine } from "./settings.js";
import { getSearchURL } from "./utils.js";
...
let allSearches = [...searchEngines, ...othersSearches, ...developmentSearches];
...
// function generateSearchEngineUrl(engineId: string): string {
//     const engine = searchEngines.find((e) => e.id === engineId);
//     if (engine) {
//         return `chrome://settings/searchEngines/add?searchEngineName=${encodeURIComponent(engine.title)}&searchEngineKeyword=${encodeURIComponent(engine.id)}&searchEngineUrl=${encodeURIComponent(engine.url)}`;
//     }
//     return '';
// }
// const addSearchEngineUrl = generateSearchEngineUrl("github");
...
chrome.omnibox.onInputEntered.addListener((text) => {
    const engine = allSearches.find((e) => e.title === text);
    if (engine) {
        const queryUrl = engine.url + encodeURIComponent(text);
        chrome.tabs.create({ url: queryUrl });
    }
